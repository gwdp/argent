...
describe("chromium blueprint factory classifications", () => {
  ...
  it("classifies an options/URN device-id mismatch as CHROMIUM_FACTORY_OPTIONS_INVALID", async () => {
    expectCode(
      await captureError(
        chromiumCdpBlueprint.factory({}, device("chromium-cdp-1111"), {
          device: device("chromium-cdp-2222"),
        })
      ),
      FAILURE_CODES.CHROMIUM_FACTORY_OPTIONS_INVALID
    );
  });

  it("classifies a malformed chromium device id as CHROMIUM_DEVICE_ID_INVALID", async () => {
    // id agrees between options and URN, so the mismatch guard passes and the
    // port parse fails — exercising the CHROMIUM_DEVICE_ID_INVALID branch.
    expectCode(
      await captureError(
